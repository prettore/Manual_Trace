...
def plot_states(trace_file, scenario, save_to, file_description, unit):
    ...
    ax.plot(trace.x, trace.y, marker='o', linestyle='', ms=1, label="")
...
def plot_animated_states(trace_file, scenario, save_to, file_description, unit):
    ...
    for i in range(0, len(trace), 3):
        ...
        # ax.set_xlim(0, 50)
        # ax.set_ylim(0, 50)
        ax.set_xlim([int(np.min(trace['x']) - .5), int(np.max(trace['x']) + 2)])
        ax.set_ylim([int(np.min(trace['y']) - .5), int(np.max(trace['y']) + 2)])
        ...
        fig.savefig(save_to + "img/gif/" + '{0:05}'.format(len(trace.iloc[0:i, :])) + ".png", bbox_inches='tight', dpi=50)
...
if __name__ == '__main__':
    ...
    #scenario = 'battleFiled_3.csv'  # (nodes move together)
    #scenario = 'battleFiled_16_4.csv'
    #scenario = 'battleFiled_16_4_closest.csv'
    scenario = 'battleFiled_16_4_intermediate.csv'
    ...
    for n in trace_node.groups:
        ...
        trace = getEquidistantPoints(trace, 25)
    ...
    #trace_nodes['node'] = trace_nodes['node'].apply(str)
    trace_nodes["node"] = trace_nodes["node"].astype(str)
    ...
    fig = px.scatter(
        ...
        hover_data=['node', 'time']
    )

    fig.show()

    px.scatter(trace_nodes, x="x", y="y", animation_frame="time", #animation_group="country",
               color="node")#, #hover_name="country")

    # colors = cycle(plotly.colors.diverging.Earth)
    # fig = go.Figure()
    # for node_id in trace_nodes.node.unique():
    #     df = trace_nodes[trace_nodes.node == node_id]
